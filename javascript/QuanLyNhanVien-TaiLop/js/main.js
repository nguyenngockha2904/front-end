...
const addEmpl = function () {
  ...
  // 3. call api toi back end, nho them dum Nv moi vao DB
  // axios luôn trả về 1 promise
  axios({
    method: 'POST',
...
const renderEmpl = function (list = empList.arr) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    htmlContent += `<tr>
      ...
      <button class="btn btn-success"  onclick="gotoDetail(${list[i].id})">View detail</button>
      </td>
	  </tr>`;
  }
...
// function get list Employees under Database
const fetchEmpls = function () {
  // method solve data when succeed
  const resolver = function (res) {
    console.log(res);
    empList.arr = res.data;
    renderEmpl();
  }
  // method solve data when fail
